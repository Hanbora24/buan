...
function financeController($scope, $state, $stateParams, mainDataService, $rootScope, $compile, $timeout, $interval, ConfigService, YearSet, GridConfig, Message) {
  ...
  $scope.PopuptabClick = function(C_CVALUE1, opt) {
    if (opt) {
      $scope.ApplyFinancialPopup($scope.PopupselectedTabId);
    } //탭 변경시 data update
  ...
  $scope.ShowFinancialPopup = function() {
    ...
    mainDataService.insertTempFinancialPlan({}).success(function(data) {
      ...
      var param = {};
      mainDataService.getFinacePriceRaiseRatio(param).success(function(data) {
  ...
  $scope.saveFinancialPopup = function() {
    ...
    mainDataService.setFinancialPlanDetail(param).success(function(data) {
      $scope.loadList();
    ...
    $.each($scope.AddListLoanItem, function(idx, Item) {
      ...
      mainDataService.addLoanItemYear(param).success(function(data) {
  ...
  $scope.ApplyFinancialPopup = function(selectedTabId) {
    ...
    mainDataService.setFinancialPlanDetail(param).success(function(data) {
      ...
      if (selectedTabId == 'FN14') {
        mainDataService.getLoanInfo(param).success(function(data) {
          console.log(data);
          var tot = {};
          $.each($scope.PopupFinanceDetailList, function(idx, Item) {
  ...
  $scope.CleanFinancialPopupInit = function() {
    ...
    for (var i = 0; i < array.length; i++) {
      ...
      mainDataService.initFinancialPlanDetailList(param).success(function(data) {
        console.log(data);
        cnt++;
        if (cnt == array.length) {
  ...
  $scope.showChartInflation = function() {
    ...
    for (var i = 0; i < $scope.inflationListChart.length; i++) {
      chartDataInflation.push({
        'CATEGORY': $scope.inflationListChart[i].NAME,
        'YYYY': $scope.inflationListChart[i].YYYY,
        'AFTERYYYY': $scope.inflationListChart[i].AFTERYYYY
      });
    }
    ;
  ...
  $scope.SaveBSPopup = function() {
    ...
    mainDataService.setFinancialStatement(param).success(function(data) {
      alertify.success('저장 되었습니다.');
      $scope.loadList();
      $scope.CloseBSPopup();
  ...
  $scope.saveAddYear = function() {
    ...
    mainDataService.addComCodeListYr2(param).success(function(data) {
  ...
  $scope.SavePLPopup = function() {
    ...
    mainDataService.setIncomeStatement(param).success(function(data) {
      alertify.success('저장 되었습니다.');
      $scope.loadList();
      $scope.ClosePLPopup();
      location.reload(true);
  ...
  $scope.SaveCFPopup = function() {
    ...
    mainDataService.setCashFlowStatement(param).success(function(data) {
      alertify.success('저장 되었습니다.');
      $scope.loadList();
      $scope.CloseCFPopup();
      location.reload(true);
  ...
  $scope.SaveOCPopup = function() {
    ...
    mainDataService.setOtherCostManagement(param).success(function(data) {
      $scope.loadList();
      $('#OCPopup').hide();
      console.log(list);
      alertify.success('저장 되었습니다.');
  ...
  $scope.AddLoanItemYear = function() {
    ...
    $.each($scope.PopupFinanceDetailList, function(idx, Item) {
      ...
      if (Item.LOAN_ITEM_CD == $scope.SelectedLoanItem.LOAN_ITEM_CD) {
        if (Item.LOAN_ITEM_YEAR != '0000') {
          item.GB_cnt = item.GB_cnt + 1;
        }
      }
      list.push(item);
      if (Item.LOAN_ITEM_YEAR == '0000') {
  ...
  $scope.dataExportToExcel = function(obj, code, gcode) {
    ...
    if (typeof code == 'undefined') {
      ...
      list.sort(function(a, b) {
        return (a.C_SCODE > b.C_SCODE) ? 1 : -1;
      });
