	 ...
	 function setGrid() {
	        ...
	        var MyGrid = pagerJsonGrid({
	            ...
	            colModel : [
	                ...
	                { name : 'FN_GBN_NM', width : 50, formatter:function(value,e,row){return getLevelName(row.FN_GBN,3)}},
	                ...
	                { name : 'STATE_CD', width : 50, edittype : 'select', formatter : 'select', editoptions : {value :'01:Y;02:N'
            			}
	                },
	                ...
	                { name : 'STATE_EVAL_METHOD_CD', width : 100,hidden : false , edittype : 'select', formatter : 'select', editoptions : {value :'0:CAP0;1:CAP1단순;2:CAP1다중;3:CAP2;4:CAP3'}},	                
...
//		             {startColumnName: 'RPN_WEIGHT_LOS1', numberOfColumns: 6, titleText: '최적투자 계획수립 LOS연관성'},
	 ...
	 $scope.loadList = function(){
	 	...
	 	$scope.initSearch = function(level){
	 		console.log(level);
	 		if(level<2)	$scope.SearchWORK_GBN = "";
	 		if(level<3) $scope.SearchAsset_GBN1 = "";
	 	}
		...
		.success(function(data){
			...
			$.each($scope.AseetLevelInfo,function(idx,Item){
				...
				if(Item.FN_CD == "F" && Item.LEVEL_STEP == 7){
					$scope.BigAssetCode.push({FN_CD: Item.FN_CD, CD: Item.LEVEL_CD ,NAME: Item.LEVEL_NM, P_LEVEL_STEP:Item.P_LEVEL_STEP, P_CLASS_CD:Item.P_CLASS_CD});
				}
				if(Item.FN_CD == "N" && Item.LEVEL_STEP == 6){
					$scope.BigAssetCode.push({FN_CD: Item.FN_CD, CD: Item.LEVEL_CD ,NAME: Item.LEVEL_NM, P_LEVEL_STEP:Item.P_LEVEL_STEP, P_CLASS_CD:Item.P_CLASS_CD });
				}
				if(Item.FN_CD == "F" && Item.LEVEL_STEP == 8){
					$scope.SmallAssetCode.push({FN_CD: Item.FN_CD, CD: Item.LEVEL_CD ,NAME: Item.LEVEL_NM, P_LEVEL_STEP:Item.P_LEVEL_STEP, P_CLASS_CD:Item.P_CLASS_CD });
				}
				if(Item.FN_CD == "N" && Item.LEVEL_STEP == 7){
					$scope.SmallAssetCode.push({FN_CD: Item.FN_CD, CD: Item.LEVEL_CD ,NAME: Item.LEVEL_NM, P_LEVEL_STEP:Item.P_LEVEL_STEP, P_CLASS_CD:Item.P_CLASS_CD });
				}
				if(Item.FN_CD == "F" && Item.LEVEL_STEP == 6){
					$scope.WorkAssetCode.push({FN_CD: Item.FN_CD, CD: Item.LEVEL_CD ,NAME: Item.LEVEL_NM , P_LEVEL_STEP:Item.P_LEVEL_STEP, P_CLASS_CD:Item.P_CLASS_CD});
				}
	 ...
	 $scope.SaveEditAssetBasePopup = function (){
		 ...
		 var param = angular.copy($scope.EditAssetBaseInfo);
		 	...
		 	mainDataService.updateAssetBase(param)
		 	.success(function(data){
    ...
    $scope.validationCheck = function() {
        ...
        switch(FN) {
            case 'F' :
                ...
                if (lenF < idx+1) {
                    ...
                    return false;
                } else { len = $scope.LevelF[idx].CD_LEN; }
                break;
            case 'N' :
                ...
                if (lenN < idx+1) {
                    ...
                    return false;
                } else { len = $scope.LevelN[idx].CD_LEN; }
                break;
            default :
        ...
        mainDataService.isAssetClassCdExists(item).success(function(obj) {
            if (obj === 1) {
                ...
                // 6. 신규 또는 수정 로직으로 이동
                if (item.isNew) { $scope.insertNewLevelCode(item); }
                else { $scope.updateLevelCode(item); }
            }
        });
    };
